...
locals {
  acl_options           = ["private", "log-delivery-write"]
  bucket_name           = var.bucket != null ? var.bucket : "s3-bucket-${random_pet.this.id}"
}
...
# Check that ACL is an allowed value
resource "null_resource" "is_acl_valid" {
  count = contains(local.acl_options, var.acl) ? 0 : "invalid" # ERROR: Invalid value provided
}
...
resource "aws_s3_bucket" "this" {
  ...
  # Server side encryption is not optional
